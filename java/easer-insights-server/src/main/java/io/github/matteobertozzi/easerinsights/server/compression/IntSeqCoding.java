...
import io.github.matteobertozzi.rednaco.bytes.encoding.BitDecoder;
import io.github.matteobertozzi.rednaco.bytes.encoding.BitEncoder;
...
public final class IntSeqCoding {
  ...
  public interface IntSeqEncoder {
    ...
    default int sliceBits(final int length, final long value, final long delta) {
      return switch (length) {
        case 0 -> 0;
        case 1 -> rleBits(length, value);
        case 2 -> linBits(length, value, delta);
        default -> minBits(length, value, delta);
      };
    }
  }
  ...
  private static final class Encoder implements IntSeqEncoder {
  ...
  private static int lengthBits(final int length) {
    ...
    final int bits = 32 - Integer.numberOfLeadingZeros(length);
    return (bits + 3) & -4;
  }
  ...
  public static int signedValueBits(final long value) {
    return value == 0 ? 2 : 1 + (64 - Long.numberOfLeadingZeros(Math.abs(value)));
  }
}
